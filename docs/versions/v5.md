# v5 — The Kitsune Integration (The Autonomous Engineer)

If you are feeling lost, don't worry! ArchFox has grown very fast. This document explains **exactly** how ArchFox works from start to finish as of version 5.
...
### Station 2: Index Codebase (Kitsune RKE)
...
It puts the text meaning into a Vector Database (**ChromaDB**) and the structural relationships (e.g. "Function A calls Function B") into a Graph Database (**Neo4j**). Now, ArchFox actually *understands* how your app is wired together.
...
ArchFox takes the PR diff and the massive Kitsune context document, and gives it to 5 different AI Agents at the exact same time:
...
The Judge Agent acts as the boss. It reads all 5 reports from the review agents, filters out the false positives, and combines them into one single **Final Report** detailing exactly what is wrong with the Pull Request.
